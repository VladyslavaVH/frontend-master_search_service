...
$(document).ready(function(){
	...
	$('.ripple-effect, .ripple-effect-dark').on('click', function(e) {
		let rippleDiv = $('<span class="ripple-overlay">'),
	...
	$(window).on('load', function() {
		$(".button.button-sliding-icon").not(".task-listing .button.button-sliding-icon").each(function() {
			let buttonWidth = $(this).outerWidth()+30;
	...
	$(window).on('load resize', function() {
		...
		if ( winwidth > 1199) {
			...
			// Notes
			$('.row').each(function() {
				let mbh = $(this).find('.main-box-in-row').outerHeight();
				let cbh = $(this).find('.child-box-in-row').outerHeight();
				if ( mbh < cbh ) {
					let headerBoxHeight = $(this).find('.child-box-in-row .headline').outerHeight();
					let mainBoxHeight = $(this).find('.main-box-in-row').outerHeight() - headerBoxHeight + 39;
			...
			// Messages Sidebar
			// let messagesList = $(".messages-inbox").outerHeight();
			// let messageWrap = $(".message-content").outerHeight();
	...
	$(window).on('load resize', function() {
		let smallFooterHeight = $('.small-footer').outerHeight();
	...
	function userMenuScrollbar() {
		$(".header-notifications-scroll").each(function() {
			let scrollContainerList = $(this).find('ul');
			let itemsCount = scrollContainerList.children("li").length;
      let notificationItems;
      
			// Determines how many items are displayed based on items height
      /* jshint shadow:true */
			if (scrollContainerList.children("li").outerHeight() > 140) {
				let notificationItems = 2;
			} else {
				let notificationItems = 3;
			}
	...
	if ($(".tabs")[0]){
		$('.tabs').each(function() {
			  ...
			  // Change Tab
			  function changeTab() {
			    let getTabId = thisTab.find('.tabs-header .active a').attr('data-tab-id');
			  ...
			  // Tab Items
			  let tabItems = thisTab.find('.tabs-header ul li');

			  // Tab Current Item
			  let tabCurrentItem = tabItems.filter('.active');
	...
	$(".keywords-container").each(function() {
		...
		let keywordInput = $(this).find(".keyword-input");
		let keywordsList = $(this).find(".keywords-list");

		// adding keyword
		function addKeyword() {
			let $newKeyword = $("<span class='keyword'><span class='keyword-remove'></span><span class='keyword-text'>"+ keywordInput.val() +"</span></span>");
		...
		keywordsList.on('resizeContainer', function(){
		    ...
		    let heightfull = $(this).css({'max-height':'auto', 'height':'auto'}).height();
    ...
    function expandAccordion (event) {
      ...
      let tabber = this.closest('.payment');
      let allTabs = tabber.querySelectorAll('.payment-tab');
	...
	/*--------------------------------------------------*/
	function qtySum(){
	    let arr = document.getElementsByName('qtyInput');
	    let tot=0;
   ...
   $(".qtyDec, .qtyInc").on("click", function() {
	...
	$(".photo-box, .photo-section, .video-container").each(function() {
		let photoBox = $(this);
		let photoBoxBG = $(this).attr('data-background-image');
  ...
  /* global ClipboardJS */
	...
	$(".share-buttons-icons a").each(function() {
		let buttonBG = $(this).attr("data-button-color");
	...
	$('.popup-tabs-nav').each(function() {
		let listCount = $(this).find("li").length;
		if ( listCount < 2 ) {
			$(this).css({
				'pointer-events': 'none'
			});
	...
	$('.indicator-bar').each(function() {
		let indicatorLenght = $(this).attr('data-indicator-percentage');
		$(this).find("span").css({
